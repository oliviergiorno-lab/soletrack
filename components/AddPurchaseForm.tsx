...
export default function AddPurchaseForm() {
  ...
  async function handleSubmit() {
    if (!selected || !form.buyPrice || !form.size) {
      alert('Veuillez remplir les champs obligatoires : sneaker, pointure et prix d\'achat')
      return
    }
    setLoading(true)
      ...
      {!open ? (
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-xl p-6">
          ...
          <div className="mb-4 relative">
            ...
            />
            {searching && (
              <div className="absolute right-3 top-9 text-zinc-500 text-xs">Recherche...</div>
            )}
            {results.length > 0 && (
              <div className="absolute z-10 w-full bg-zinc-800 border border-zinc-700 rounded-lg mt-1 overflow-hidden shadow-xl">
                {results.map(product => (
                  ...
                  >
                    {product.thumbnail && (
                      <Image src={product.thumbnail} alt={product.model} width={48} height={48} className="object-contain rounded" />
                    )}
                    <div>
                      <div className="text-sm text-white font-medium">{product.brand} {product.model}</div>
                      <div className="text-xs text-zinc-400">{product.colorway} · {product.sku}</div>
                    </div>
                  </button>
                ))}
              </div>
          ...
          {selected && (
            <div className="grid grid-cols-3 gap-4 mb-4">
              {[
                ...
                { name: 'buyPrice', label: 'Prix achat (€) *', placeholder: '0.00', type: 'number' },
                { name: 'fees', label: 'Frais annexes (€)', placeholder: '0.00', type: 'number' },
              ].map(field => (
                <div key={field.name} className="flex flex-col gap-1">
                  <label className="text-xs text-zinc-400 uppercase tracking-wider">{field.label}</label>
                  <input
                    name={field.name}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-zinc-400 uppercase tracking-wider">Fournisseur</label>
                <select
                  name="platform"
                  value={form.platform}
                  onChange={handleChange}
